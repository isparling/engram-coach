...
That file is the coaching view and is normally the **complete** picture: every
active claim, including medical ones, plus coach-side persona-fit reasoning.
Medical facts are deliberately included — medications, airway, and load
conditions materially change what should be prescribed, and reasoning without
them is reasoning from a knowingly incomplete model.

`ATHLETE_PROFILE.md` is the athlete-facing render of the same records: it omits
persona-fit reasoning, which is written for you rather than for them. Read it
only when you need to know what the athlete themselves sees.

The one thing the coach view can lack is a claim the athlete scoped
`visibility: private`. That withholding is intentional and is not to be worked
around by reading `ATHLETE_PROFILE.md` to recover it.
